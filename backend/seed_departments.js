...
async function seed() {
  for (const [deptName, subDepts] of Object.entries(data)) {
    // Upsert Department
    let department = await prisma.masterItem.findFirst({
      where: { category: 'department', name: deptName }
    });

    if (!department) {
      department = await prisma.masterItem.create({
        data: { category: 'department', name: deptName }
      });
    ...
    // Upsert Sub Departments
    for (const subDeptName of subDepts) {
      let subDepartment = await prisma.masterItem.findFirst({
        where: { category: 'subDepartment', name: subDeptName, parentId: department.id }
      });

      if (!subDepartment) {
        await prisma.masterItem.create({
          data: { category: 'subDepartment', name: subDeptName, parentId: department.id }
        });
        console.log(`  Created Sub Department: ${subDeptName}`);
      }
    }
  }
}
